...
| `password.txt`     | リポジトリの暗号化パスワード（※安全なパスワードに変更してください） |
...
1. `password.txt` をメモ帳等で開き、好きなパスワード（暗号化用）を入力して保存します。
2. `config.ps1` を開き、`$script:ResticRepository` を実際のNASの共有パス（例: `\\192.168.1.100\backup\restic-repo`）に書き換えます。
